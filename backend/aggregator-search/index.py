...
SEARCH_TOPICS = [
    ...
    },
    {
        "id": "smr",
        "label": "СМР и отделка",
        "queries": ["строительно-монтажные работы подряд", "монтаж оборудования металлоконструкций", "отделочные работы ремонт помещений"],
...
TOPIC_KEYWORDS = {
    ...
    "smr": [
        ...
        "реконструкц", "капитальный ремонт", "отделочн", "отделка", "ремонт помещен",
        "штукатурн", "малярн", "облицовк", "напольн", "потолочн", "кровельн",
        "фасадн", "внутренние работы", "общестроительн",
    ],
    "mining": [
...
def process_topic(topic: dict) -> list:
    ...
    for t in all_tenders:
        ...
        # Фильтр по тематике — проверяем title + description + customer
        if topic["id"] != "custom":
            check_text = " ".join([
                t.get("title") or "",
